...
test('Load Blink example and check code', async ({ page }) => {
    ...
    await page.locator('div').filter({ hasText: /^Blink$/ }).nth(1).click();

    // Check code
    await page.locator('button').filter({ hasText: /^code$/ }).click();
	await expect(page.locator('.view-lines')).toContainText("void leaphyProgram() { while (true) { digitalWrite(13, HIGH); delay(1000); digitalWrite(13, LOW); delay(1000); }}void setup() {pinMode(13, OUTPUT); leaphyProgram();}void loop() {}");
	...
	await expect(page.locator('.view-lines')).toContainText("delay(500); digitalWrite(13, LOW); delay(500);");

    // Save the robot
    await page.getByRole('button', { name: 'Save' }).click();
    await page.getByPlaceholder('Give your download a name').fill('MyModifiedBlink');
