...
class Arduino {
    ...
    async serialMonitor(outputStream: WritableStream) {
        ...
        try {
            ...
            const pipePromise = readableStream.pipeTo(outputStream, {
            ...
            pipePromise
                .catch((error) => {
                    if (error.toString().includes("Upload started")) {
                        ...
                        error.toString().includes("The device has been lost.")
                    ) {
                ...
                .then(async () => {
                    ...
                    await this.port
                        ...
                        .catch(async () => {
                            ...
                            const port =
                                await this.robotWiredState.requestSerialPort(
                                    false,
                                );
                            if (!port) {
                                this.port = null;
    ...
    async upload(program: Record<string, string>) {
        if (this.isUploading) throw new Error("Arduino is already uploading");

        if (this.port == null) throw new Error("No device selected");
        ...
        await upload.upload(program).catch(async (err) => {
            await this.close();
            throw err;
        });
        await this.close();
