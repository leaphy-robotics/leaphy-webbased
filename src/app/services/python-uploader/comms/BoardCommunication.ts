...
async function readResponse(
    ...
    while (true) {
        ...
        fullReadResponse = new Uint8Array([
            ...fullReadResponse,
            ...readResponse.value,
        ]);
        if (
            fullReadResponse[fullReadResponse.length - 1] ===
            RAW_END_OF_RESPONSE
        ) {
            fullReadResponse = fullReadResponse.slice(
                0,
                fullReadResponse.length - 1,
            );
            stdOut = decoder.decode(
                fullReadResponse.slice(0, fullReadResponse.indexOf(EOF)),
            );
            stdErr = decoder.decode(
                fullReadResponse.slice(
                    fullReadResponse.indexOf(EOF),
                    fullReadResponse.lastIndexOf(EOF) - 1,
                ),
            );
            break;
        }
