...
export class PackageManager {
    ...
    public async flashLibrary(url: string) {
        ...
        if (!url) {
            ...
            if (
                await this.checkLibraryVersion(
                    version,
                    PackageManager.getLibraryName(url),
                )
            ) {
                return;
            }
            ...
            for (const file of content) {
                ...
                if (filename.endsWith(".py") || filename.endsWith(".py/")) {
                    ...
                    const response = await fetch(
                        `${environment.backend}/minify/python`,
                        {
                            method: "POST",
                            headers: {
            ...
            await rm(
                writer,
                reader,
                "/lib/" + PackageManager.getLibraryName(url) + ".json",
            );
            json = { version: version };
            await put(
                writer,
                reader,
                PackageManager.getLibraryName(url) + ".json",
