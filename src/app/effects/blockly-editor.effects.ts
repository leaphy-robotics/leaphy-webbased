...
import {
    LISTS,
    ListSerializer,
} from "@leaphy-robotics/leaphy-blocks/categories/all";
import {
    categoryStyles,
    componentStyles,
    defaultBlockStyles,
} from "@leaphy-robotics/leaphy-blocks/theme/theme";
...
export class BlocklyEditorEffects {
    constructor(
        ...
        this.appState.changedLanguage$
            .pipe(filter((language) => !!language))
            .subscribe(() => {
                this.blocklyState.setWorkspaceStatus(
                    WorkspaceStatus.SavingTemp,
                );
            });
        ...
        combineLatest([
            ...
            .subscribe(
                ([
                    ...
                    setTimeout(
                        () =>
                            this.blocklyState.setIsSideNavOpen(
        ...
        this.blocklyState.workspace$
            ...
            .subscribe((workspace) => {
                ...
                workspace.addChangeListener(async () => {
                    this.codeEditorState.setCode(
                        Arduino.workspaceToCode(
                            workspace,
                            this.appState.getSelectedRobotType().id,
                        ),
                    );
                    this.blocklyState.setWorkspaceJSON(
                        JSON.stringify(
